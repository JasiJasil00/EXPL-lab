    ...
    switch(t->nodetype){
        ...
        case NODE_VAR:
            if(t->Gentry!=NULL && t->Gentry->array!=NODE_VAR){
                ...
                yyerror("ID type not var");
            }else if(t->Gentry==NULL){
                yyerror("no present in symbol table");
            }
            break;
        case NODE_ARRAY:
            if(t->Gentry!=NULL && t->Gentry->array!=NODE_ARRAY){
                if(t->Gentry!=NULL)printf("%s %d %d\n",t->varname, t->Gentry->size,t->Gentry->sizecol);
                yyerror("ID type not array");
            }else if(t->Gentry==NULL){
                yyerror("no present in symbol table");
            }
            break;
        case NODE_2D_ARRAY:
            if(t->Gentry!=NULL && t->Gentry->array!=NODE_2D_ARRAY){
                yyerror("ID type array, not 2d array");
            }else if(t->Gentry==NULL){
                yyerror("no present in symbol table");
            }
            break;
